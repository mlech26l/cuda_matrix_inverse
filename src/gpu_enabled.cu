...
int lup_matrix_inverse_gpu(float *d_A, float* d_A1,int n)
{
  preload_device_properties(n);
  ...
  unload_device_properties();

  return 1;
}

int lup_matrix_inverse_cpu(float *A, float* A1,int n)
{
  cudaEvent_t start, stop;
  float milliseconds=0;
  cudaEventCreate(&start);
...
int lup_decomposition_gpu(float *A, int *pivot, int n) {
  ...
  for (int k = 0; k < n; k++) {
    ...
    max_entry pivoting = find_pivot_semi_gpu(A,n,k);
